...
export const PizzaBlock: React.FC<pizzaType> = ({title, price, imageUrl, sizes, types}) => {
    ...
    const doughsNames = ['тонкое', 'традиционное']
                ...
                <ul>
                    {
                        types.map((type, index) =>
                            <li onClick={() => setActive(index)}
                                className={index === activeDoughType ? 'active' : ''}
                                key={index}>{doughsNames[type]}</li>)
                    }
                </ul>
                <ul>
                    {
                        sizes.map((size, index) =>
                            ...
                            <li onClick={() => setActivePizzasSize(index)}
                                className={index === activePizzasSize ? 'active' : ''}
                                key={index}>{size}</li>)
                    }
                </ul>
            </div>
